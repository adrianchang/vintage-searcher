...
| `GET /story/:id` | — | Public, unsigned, shareable story card (no email/token — meant to be forwarded to strangers) — see Sharing below |
...
- **Email entry point**: the digest always leads with a "Try It On" picker (`buildTryOnPickerHtml`) *before* the intro line, regardless of whether the user has a photo yet — deliberately front-loaded so the pick doesn't require reading the whole email first. Each item is a real gold-filled button, not a text link — click-through was only ~24% of recipients as of 2026-08-27, traced partly to the original design (a quiet text-link list) not reading as clickable; still deliberately no item thumbnails, since pictures risked picture-then-bounce behavior that skips the actual stories below. Copy is intentionally short ("Pick one — one try per email") and — fixed 2026-08-28 — no longer says "today," since the quota moved to per-digest-batch on 2026-08-25 and the old "one shot today" wording had gone stale relative to that. Each button links to `GET /tryon?e=&s=&t=` (HMAC-signed like vote/click links, `buildTryOnUrl` in `email.ts`).
...
- **Rate limit — per digest batch, not per calendar day** (changed 2026-08-25): originally one successful try-on per UTC day; switched because it created a real dead zone — skip a day's email and you'd lose that day's try entirely, or use today's try-on early and be locked out even though tomorrow's picks hadn't arrived yet. Now every digest send gets its own `batchId` (`StoryDelivery.batchId`, one shared id generated per user per scan in `scan.ts`, `crypto.randomUUID()`), and quota is "one try-on per batch" — `GET /tryon` looks up the clicked item's `StoryDelivery` to find its batch, then checks for an existing `status: "done"` `TryOn` with that same `batchId`. Unused batches never expire, so a user who ignored several days of emails can spend all of them in one sitting — deliberate, confirmed with the user, not a bug. A **failed** generation doesn't consume the batch's allowance. Re-clicking the same item's link for an already-used batch shows the cached result instead of regenerating; clicking a *different* item from an already-used batch shows an "already used this pick's try-on" page — but a different, still-unused batch's items remain fully clickable. Historical `StoryDelivery`/`TryOn` rows from before this column existed were backfilled to `userId + day(sentAt/createdAt)` (see the `20260825000000_add_tryon_batch_quota` migration), reconstructing the old one-batch-per-day behavior for old data without needing a real batch grouping.
...
- **Loading screen + `GET /tryon/generate`** (added 2026-08-24): generation is a real Gemini call (routinely 10-20+ seconds), and the original design left the browser on a blank tab that whole time. `GET /tryon` now renders a loading page immediately whenever generation is actually needed (a spinner + "usually takes 10-20 seconds"), whose own script calls the new `GET /tryon/generate` (same HMAC auth, re-validates everything independently — story, photo, and the daily limit again, since it's a separate request that two tabs or a retried fetch could both hit) to do the real work and create the `TryOn` row. On success the loading page just navigates to the exact same `/tryon` URL, which now finds a `done` row and renders the cached result instantly — no result-building logic is duplicated between the two routes. Verified locally end-to-end: the loading page returns in ~0.02s with real content, `/tryon/generate` completes a real generation in ~10s, and the follow-up `/tryon` reload returns the finished result in ~0.02s.
- **Bilingual `/tryon` pages** (added 2026-08-24): the upload form and the loading screen use `TRYON_LABELS[lang]` (module-level, near the top of `server.ts`, mirrors `email.ts`'s `LABELS` pattern) keyed off `user.language`. Other `/tryon` pages (the "already used today" message, the various error states) are still English-only — not yet extended to match, a known gap if you're looking for it.
...
### Sharing (`GET /story/:id` in `server.ts`, added 2026-08-27)

A public, unsigned page per `Story` — deliberately different from every other `/story`-adjacent-sounding route in this file, which are all HMAC-gated to one specific recipient. This one is meant to be forwarded to strangers, so the URL carries no email/token, just the `Story.id` (an unguessable cuid, same "opaque ID is enough" pattern as `/evaluations/:id/image`).

- **Card layout**: item photo (same `hasProcessedImage` fallback as everywhere else), era tag, item name, hook as a pull-quote, full `mainStory`, an eBay CTA linking straight to the listing (no `/go` click-tracking — that route requires a known recipient email, which an anonymous visitor doesn't have), a native-share button, and a "get your own daily finds" sign-up CTA — the page's job is to convert the *viewer*, not just serve the *sharer*.
- **Open Graph + Twitter Card meta tags** (`og:title`/`og:description`/`og:image`, `twitter:card=summary_large_image`) so the link shows a real preview card (image + hook) when pasted into iMessage, LINE, X, Slack, etc., instead of a bare URL. `og:description` is the hook text, trimmed to ~160 chars for card-preview length.
- **Share button**: `navigator.share()` (the OS-level share sheet — LINE, Messages, WhatsApp, Mail, whatever's installed) when available, falling back to `navigator.clipboard.writeText()` + a "Link copied!" message on desktop browsers that don't support the Web Share API.
- **Entry point**: a small "Share this find →" text link under each item in the digest email (`buildStoryShareUrl` in `email.ts`) — deliberately a quiet text link, not another button, since each item card is already dense (image, price, vote row, size line, eBay CTA, hook, story, style guide); a per-item share *button* would add more visual competition than the feature is worth. No email-level/single "share the whole digest" link — per-item was chosen because a specific, cool piece is a much easier thing to actually forward than "check out this newsletter."
- **Bilingual**: renders in the `Story`'s own `language` field directly (no query param needed — the story already knows what language it was generated in), via a small local `STORY_SHARE_LABELS` map in `server.ts` (same pattern as `TRYON_LABELS`).
- **Not built**: no view/share-count tracking yet, and personal try-on photos are deliberately excluded from public sharing for now — only the item's story/photo, never a user's own uploaded photo or generated try-on render, since that's a real privacy line (a stranger's face becoming a public URL) that wasn't part of this request.
...
- `StoryDelivery` — tracks what has been sent to each user. Prevents resending. `batchId` groups every item from the same digest send (see Virtual Try-On's per-batch rate limit).
...
- `TryOn` — one row per virtual try-on attempt (`status: "done" | "failed"`, bytes only on success). `batchId` ties it to the `StoryDelivery` batch it "spent" — see Virtual Try-On above for the per-batch rate limit this enables.
...
**Hero image background removal:** deferred to send-time (`scan.ts`, changed 2026-09-21) — runs only for the top-3 listings actually selected for a user's digest, not every filtered candidate that gets identified. (Originally ran at evaluation-creation time for every candidate; an August 2026 billing audit found only ~35% of evaluated listings and ~8% of generated stories ever got delivered to anyone, so background removal — the same fixed cost regardless of whether the listing shipped — was running 2-3x more often than needed. Deferring it to the `toSend` loop, right before `sendDigestEmail`, cuts that waste to zero while keeping the same "once per listing, forever" cache semantics; the trigger point moved, not the caching model.) `runBackgroundRemoval` (in `evaluate.ts`) sends the listing's first image (`listing.imageUrls[0]` only — the other listing photos are untouched) to `gemini-3.1-flash-image` via `generateContent` (same API key, same throttle queue as identification/valuation/story — no Vertex AI setup) requesting a pure white studio backdrop. (A side-by-side test on 2026-08-10 initially picked a soft gray over white/cream; overridden back to white after reviewing real production output the same day — 8 evaluations from that window carry the gray version permanently, since results are never regenerated once cached.) Best-effort only — never blocks sending on failure. Result is stored as raw bytes on `Evaluation.heroImageBytes`/`heroImageMimeType`/`hasProcessedImage`, generated once per listing forever (same caching tier as identification, not per story/config — the same photo is reused across every archetype/language variant, and across every future scan that selects this listing again). Bytes are deliberately excluded (`omit`) from the routine per-user cache-check query in `scan.ts` since they're a multi-hundred-KB payload hit on every (user, listing) pair; only `GET /evaluations/:id/image` fetches them, falling back to the original `imageUrl` if generation never succeeded or never ran (legacy rows, failures, or simply never selected for a digest yet).
