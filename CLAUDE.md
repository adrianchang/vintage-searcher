...
**Compact layout (2026-08-13):** price and the 👍/👎 vote links share one quiet single-line row (plain text on the page background, no card) instead of separate full-width blocks — research-backed call: the isolation/Von Restorff effect means a price card styled like the eBay button competes with it rather than reinforcing it, so the CTA is now the only bold dark element per item. This also moves voting earlier (right after image + price, before the story) — a closer-to-gut-reaction signal than the old post-story feedback card; worth watching in vote data. `styleGuide` renders via `firstSentence()` (trims to the first sentence, hard-capped) since the generation prompt now asks for one tight sentence — older cached stories written under the old (longer) prompt get trimmed at render time, never regenerated.
