...
// waiting on a page load, and volume is already self-limited to once/day/user
// (see startOfUtcDay + the daily-limit check in server.ts's GET /tryon).
...
/**
 * Start of the current UTC calendar day — same day-boundary convention as
 * archetypes.ts's dayIndex, so "once per day" means the same thing everywhere
 * in this codebase. Callers use this as the `createdAt` floor when checking
 * for an existing *successful* ("done") TryOn today; failed attempts are
 * excluded from that query so a generation error never costs the user their
 * day's try.
 */
