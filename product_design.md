...
| 2026-08-27 | Public shareable story pages ship (`GET /story/:id`) with a "Share this find →" text link per email item — no referral/credit system, no per-item share *buttons* (rejected for adding clutter to an already-dense item card), and personal try-on photos deliberately excluded from public sharing | User explicitly said no referral rewards needed, just "let people share our stuff easily." A real public webpage (not mailto:/LINE/X links baked into the email) was chosen specifically because it unlocks the OS-level native share sheet (`navigator.share()` — any installed app, not 3 hardcoded ones) and Open Graph preview cards, neither of which work from inside an email. Try-on photos scoped out because a stranger's own face becoming a public URL is a real privacy line the request didn't actually ask to cross |
| 2026-09-06 | Try-on picker gets item thumbnails back — each gold button now shows the item's background-removed cutout image (same `hasProcessedImage`-aware source as the item cards) beside the name/ordinal, reversing the 2026-08-22 text-only decision | The text-only bet (avoid picture-then-bounce, let the story content earn the try-on click) didn't move click-through on its own across multiple copy/button redesigns (2026-08-23, 2026-08-28). User's read: that was too idealistic — the item's own photo should be what draws someone into trying it on, i.e. let try-on drive interest in the content rather than the other way around |
